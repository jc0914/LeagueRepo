...
namespace e.Motion_Gangplank
{
    internal class Program
    {
        ...
        private static void OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender.IsMe && args.Slot == SpellSlot.Q && E.IsReady(200) && args.Target.Name == "Barrel")
            {
                ...
                if (Config.Item("combo.triplee").GetValue<bool>() && barrelsInRange.Any())
                {
                    ...
                    {
                        Utility.DelayAction.Add(Helper.GetQTime(args.Target.Position) - 100 - Game.Ping / 2, () => InvokeTriplePlacement(barrel, AllBarrel));
                    }
        ...
        private static void InvokeTriplePlacement(Barrel connectingBarrel, IEnumerable<Barrel> hitTest)
        {
            ...
            IEnumerable<Obj_AI_Hero> invokedEnemies = HeroManager.Enemies.Where(e => e.Position.Distance(connectingBarrel.GetBarrel().Position) < 1370 && !hitTest.Any(b => b.GetBarrel().Position.Distance(e.Position) < 340));
            foreach (Obj_AI_Hero enemy in invokedEnemies)
            {
        ...
        private static void TriplePlacement(Obj_AI_Hero enemy, Vector3 position)
        {

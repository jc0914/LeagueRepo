    ...
    class Braum : Base
    {
        ...
        private void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (W.IsReady() && args.SData.MissileSpeed > 0)
            {
                ...
                {
                    if ((args.Target == null && OktwCommon.CanHitSkillShot(ally, args.Start, args.End, args.SData)) || OktwCommon.GetIncomingDamage(ally) > ally.Health * Config.Item("Edmg", true).GetValue<Slider>().Value * 0.01)
                    {
                        if (E.IsReady())

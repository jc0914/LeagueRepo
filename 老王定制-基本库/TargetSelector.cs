namespace LeagueSharp.Common
{
    ...
    public class TargetSelector
    {
        ...
        {
            var p = 1;
            if (_configMenu != null && _configMenu.Item("TargetSelector" + hero.NetworkId + "Priority") != null)
            {
                p = _configMenu.Item("TargetSelector" + hero.NetworkId + "Priority").GetValue<Slider>().Value;
            }
            ...
            CustomEvents.Game.OnGameLoad += args =>
                {
                    ...
                    foreach (var enemy in HeroManager.Enemies)
                    {
						...
						config.AddItem(
                            new MenuItem("TargetSelector" + enemy.NetworkId + "Priority",  $"{enemy.ChampionNameWithTab()}({enemy.Name.ToGBK()})")
								.SetShared()
                                .SetValue(
                                    new Slider(
                        ...
                        {
                            config.Item("TargetSelector" + enemy.NetworkId + "Priority")
                                .SetValue(
                                    new Slider(
                                        autoPriorityItem.GetValue<bool>() ? GetPriorityFromDb(enemy.ChampionName) : 1,
                                        5,
        ...
        public static void SetPriority(Obj_AI_Hero hero, int newPriority)
        {
            ...
            var p = _configMenu.Item("TargetSelector" + hero.NetworkId + "Priority").GetValue<Slider>();
            p.Value = Math.Max(1, Math.Min(5, newPriority));
            _configMenu.Item("TargetSelector" + hero.NetworkId + "Priority").SetValue(p);
        }
        ...
        private static void autoPriorityItem_ValueChanged(object sender, OnValueChangeEventArgs e)
        {
            ...
            foreach (var enemy in HeroManager.Enemies)
            {
                _configMenu.Item("TargetSelector" + enemy.NetworkId + "Priority")

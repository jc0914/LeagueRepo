    ...
    public static class Orbwalking
    {
        ...
        public class Orbwalker : IDisposable
        {
            ...
            public Orbwalker(Menu attachToMenu)
            {
				...
				_config.AddItem(
					new MenuItem("AutoExtraWindup", "自动设定额外延时[默认80]").SetShared().SetValue(true));
            ...
            private void GameOnOnGameUpdate(EventArgs args)
            {
                try
                {
					...
					//修改的内容：自动设置windUp
					var windUp = _config.Item("ExtraWindup").GetValue<Slider>().Value;
					if (_config.Item("AutoExtraWindup").GetValue<bool>())
					{
						var additional = 0;
						if (Game.Ping >= 100)
						{

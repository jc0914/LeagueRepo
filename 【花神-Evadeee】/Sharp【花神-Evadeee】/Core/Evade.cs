    ...
    internal class Evade
    {
        ...
        private void Game_OnGameLoad(EventArgs args)
        {
            try
            {
				...
				Game.PrintChat("<font color=\"#00FFFF\" >鑺辩 </font><font color=\"#CCFFFF\" >FloraEvadeee</font> - " +
                   "<font color=\"#EEEE00\" >妫€鏌ョ増鏈洿鏂帮細 " + Assembly.GetExecutingAssembly().GetName().Version + "</font>");

				   
                Game.PrintChat(devModeOn ? "<b>Evadeee: Developer Mode On</b>" : "<b>Evadeee: Loaded!");

                menu = new Menu("花神VIP；Evadeee", "花神VIP；Evadeee", true);
                ...
                mainMenu.AddItem(new MenuItem("DodgeDangerous", "请勿开启").SetValue(false));
                ...
                mainMenu.AddItem(new MenuItem("PotsEvade", "   注入内核？").SetValue(true));
				mainMenu.AddItem(new MenuItem("DuressEvade","    强制躲避？").SetValue(true));
				mainMenu.AddItem(new MenuItem("FastEvade","    快速躲避？").SetValue(true));
				mainMenu.AddItem(new MenuItem("Packets","     Packets").SetValue(false));
				mainMenu.AddItem(new MenuItem("2",""));
				mainMenu.AddItem(new MenuItem("VersionCheck","    检测版本更新").SetValue(true));
				mainMenu.AddItem(new MenuItem("VER","    7.0.0.0(最后版本)"));
				mainMenu.AddItem(new MenuItem("",""));
				mainMenu.AddItem(new MenuItem("Hint","    请勿搭配任何躲避缓存 Thank！"));
				mainMenu.AddItem(new MenuItem("6","    外传狗-继续传啊 倒卖狗 继续倒卖啊 老子不更新了 你继续更啊"));
				mainMenu.AddItem(new MenuItem("3","    MDZZ 弱智狗 88  倒卖狗 继续倒卖哦！！ 我等你更新8.0.0.0呢"));
				mainMenu.AddItem(new MenuItem("4","    依晴姐-小弟多有得罪.还望您见谅"));
                ...
                miscMenu.AddItem(new MenuItem("AdvancedSpellDetection", "Advanced Spell Detection").SetValue(true));
                ...
                //miscMenu.AddItem(new MenuItem("AllowCrossing", "Allow Crossing").SetValue(true));
                //miscMenu.AddItem(new MenuItem("CalculateHeroPos", "Calculate Hero Position").SetValue(true));
                miscMenu.AddItem(new MenuItem("EvadeMode", "默认即可")
                    .SetValue(new StringList(new[] {"0", "Very Smooth", "Fastest", "Hawk", "Kurisu", "GuessWho"}, 0)));
                miscMenu.Item("EvadeMode").ValueChanged += OnEvadeModeChange;
                miscMenu.AddItem(new MenuItem("ResetConfig", "不躲避点我吧").SetValue(false));
                ...
                if (devModeOn)
                {
                    var rootTestMenu = new Menu("Evadeee Tester", "EvadeeeTester", true);
                ...
                Console.WriteLine("Evadeee Loaded");
            }
            catch (Exception e)
            {
                Console.WriteLine(e);

...
namespace EloFactory_Jinx
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config = new Menu(ChampionName + " 老王漢化", ChampionName + " 老王漢化", true);

            Config.AddSubMenu(new Menu("Orbwalking", "Orbwalking"));

            var targetSelectorMenu = new Menu("Target Selector", "Target 目標选择");
            ...
            Config.SubMenu("Misc").SubMenu("Skin Changer").AddItem(new MenuItem("Jinx.SkinChangerName", "Skin choice").SetValue(new StringList(new[] { "Classic", "Mafia", "Firecracker","Slayer","Star Guardian"})));
            ...
            Config.SubMenu("我要超神 ").AddItem(new MenuItem("kaiqi","开启超神").SetValue(false));
            Config.SubMenu("我爱老王 ").AddItem(new MenuItem("woai","开启五杀模式").SetValue(false));
            ...
            Game.PrintChat("<font color='#FF99CC'><b>鑰佺帇瀹氬埗 鏃犳晫閲戝厠绲蹭辅 娉ㄥ叆鎴愬姛</b></font><font color='#FF3300'><b>鑰佺帇VIP鑵虫湰浜ゆ祦缇や辅348902882</b></font>");
        ...
        {
            Player.SetSkin(Player.BaseSkinName, Config.Item("Jinx.SkinChanger").GetValue<bool>() ? Config.Item("Jinx.SkinChangerName").GetValue<StringList>().SelectedIndex : Player.BaseSkinId);
        ...
        public static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base unit, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (Config.Item("Jinx.AutoEWhenEnemyCastAAM").GetValue<bool>() && (unit.IsValid<Obj_AI_Hero>() && !unit.IsValid<Obj_AI_Turret>()) && unit.IsEnemy && args.Target.IsMe && args.SData.IsAutoAttack() && E.IsReady() && Player.Distance(unit) < 300)
            {
                E.Cast(Player.ServerPosition, true);
            }

            if (Config.Item("Jinx.AutoEWhenEnemyCastAAR").GetValue<bool>() && (unit.IsValid<Obj_AI_Hero>() && !unit.IsValid<Obj_AI_Turret>()) && unit.IsEnemy && args.Target.IsMe && args.SData.IsAutoAttack() && E.IsReady() && Player.Distance(unit) > 300 && Player.Distance(unit) <= E.Range && Player.CountEnemiesInRange(300) == 0)
            {
                E.Cast(unit, true, true);
            }
            ...
            if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
            {
                if (Config.Item("Jinx.UseECombo").GetValue<bool>() && (unit.IsValid<Obj_AI_Hero>() && !unit.IsValid<Obj_AI_Turret>()) && unit.IsEnemy && args.Target.IsMe && args.SData.IsAutoAttack() && E.IsReady() && Player.Distance(unit) < 300)
                {
                    E.Cast(Player.ServerPosition, true);
                }

                if (Config.Item("Jinx.UseECombo").GetValue<bool>() && (unit.IsValid<Obj_AI_Hero>() && !unit.IsValid<Obj_AI_Turret>()) && unit.IsEnemy && args.Target.IsMe && args.SData.IsAutoAttack() && E.IsReady() && Player.Distance(unit) > 300 && Player.Distance(unit) <= E.Range && Player.CountEnemiesInRange(300) == 0)
                {
                    E.Cast(unit, true, true);
                }
            }
